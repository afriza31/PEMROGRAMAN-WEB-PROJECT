  ...
  Alpine.store("cart", {
    ...
    add(newItem) {
      ...
      if (!cartItem) {
        ...
        //jika barang sudah ada, cek apakah barangnya beda atau sama dengan yang ada di cart
    ...
    remove(id) {
      // ambil item yang ingin dihapus berdasarkan id nya
      ...
      if (cartItem.quantity > 1) {
        ...
        this.items = this.items.map((item) => {
          ...
          if (item.id !== id) {
            ...
            // jika barang sudah ada, kurangi quantity dan totalnya
            item.quantity--;
            item.total = item.price * item.quantity;
            this.total -= item.price;
            this.quantity--;
